...
class ThirdActivity : AppCompatActivity(), GestureDetector.OnGestureListener {
    ...
    private val currentTeamIndex = 2 // ThirdActivity es Rams (índice 2)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Aquí puedes agregar código específico para Rams
        setupRamsContent()
    }

    private fun setupRamsContent() {
        // Configurar contenido específico de Rams
    ...
    private fun setupTeamNavigation() {
        // Navegación a info detallada al hacer click en la card activa (Rams)
        findViewById<androidx.cardview.widget.CardView>(R.id.cardRamsContainer).setOnClickListener {
            val intent = Intent(this, RamsInfoActivity::class.java).apply {
                putExtra("TEAM_NAME", "Los Angeles Rams")
                putExtra("TEAM_COLOR", "#003594")
                putExtra("TEAM_BACKGROUND", "rams_image")
                putExtra("TEAM_INDEX", 2)
            }
            startActivity(intent)
        }
